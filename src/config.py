...
私は応用研究者である。したがって論文選定は以下の原則を最優先する:

1. **応用接続を必須化**: LLM・AI・機械学習の研究は、社会データ・都市・交通・社会
   ネットワーク・政策・人間行動 のいずれかへの応用可能性が明示的または容易に
   想像できる場合に高評価。純粋な能力向上・ベンチマーク改善・理論解析のみの
   論文は、たとえLLM関連でも低〜中評価に留める。

2. **クロスドメイン研究を優遇**: 「LLM × 交通」「エージェント × 都市」「SNS分析 ×
   AI」のように、AI技術と私の応用領域を橋渡しする研究は、たとえ完成度が低くても
   上位選定する価値がある。

3. **応用研究者の視点**: 私は理論研究者ではない。実データで何ができるか、社会で
   どう使えるかが見えない論文は、技術的に優れていても優先度を下げる。
