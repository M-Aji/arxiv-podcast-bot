...
1. [Stateful Online Monitoring Catches Distributed Agent Attacks](https://arxiv.org/abs/2605.31593) — score 6.5  
   邦訳: ステートフルなオンライン監視が分散エージェント攻撃を検知する
2. [LongTraceRL: Learning Long-Context Reasoning from Search Agent Trajectories with Rubric Rewards](https://arxiv.org/abs/2605.31584) — score 6.5  
   邦訳: 検索エージェント軌跡とルーブリック報酬を用いた長文脈推論の強化学習
3. [Disagreeing Rationales: Rethinking Classification and Explainability Evaluation in Hate Speech Detection](https://arxiv.org/abs/2605.31563) — score 6.5  
   邦訳: 異なる根拠：ヘイトスピーチ検出における分類と説明可能性評価の再考
4. [Effects of Vertex Merging & Splitting on Large Coauthorship Networks: A Counterfactual Analysis](https://arxiv.org/abs/2605.31555) — score 6.5  
   邦訳: 大規模共著ネットワークにおける頂点の統合と分割の効果：反事実分析
5. [Giving Sensors a Voice: Multimodal JEPA for Semantic Time-Series Embeddings](https://arxiv.org/abs/2605.31580) — score 5.5  
   邦訳: センサーに声を与える：セマンティック時系列埋め込みのためのマルチモーダルJEPA
...
**邦訳:** 拡散モデルのグラフテキスト生成における逐次復号化軌跡分析：何が最初に復号化されるのか
**Authors:** Qing Wang, Jacob Devasier, Chengkai Li
**Category:** cs.CL
**Published:** 2026-05-29
**arXiv:** https://arxiv.org/abs/2605.31564

**評価理由:** マスク付き拡散言語モデルのグラフテキスト生成への応用研究であり、方法論的な新規性（lambda-scaled structural decoding）と生成メカニズムの分析は興味深いが、応用領域が限定的（グラフ構造化データのテキスト化）で、都市・交通・社会データ分析への直接的な接続が明確でない。純粋な技術改善に近い。
...
**邦訳:** 何が足りないのか？質問応答による隠れ状態の探査
**Authors:** Chu Fei Luo, Samuel Dahan, Xiaodan Zhu
**Category:** cs.CL
**Published:** 2026-05-29
**arXiv:** https://arxiv.org/abs/2605.31561

**評価理由:** LLMの推論メカニズムと自己修正能力に関する理論的研究であり、『強く興味あり』テーマに該当するが、応用接続（社会データ、都市、交通、政策など）が明示的でなく、純粋な能力分析・メカニズム解析に留まっているため中程度の評価。ただし、自己診断と修正のギャップは政策支援AIやLLM応用システムの実装上の課題として参考になる可能性がある。
...
**評価理由:** テーブル質問応答という自然言語処理タスクの改善論文だが、応用接続（都市・交通・社会データ・政策など）が明示的でなく、純粋な技術改善に近い。ただしLLMの実用的な表現方法の工夫であり、社会データ分析での表形式データ処理に間接的に関連する可能性があるため、中程度の評価。
...
**評価理由:** LLMの個人化評価という技術的に興味深いテーマだが、応用接続（都市・交通・社会データ・政策など）が明示的でなく、主にLLMの評価方法論の改善に留まっているため、応用研究者の優先度は中程度。ユーザー履歴からの学習という方法論は参考になるが、社会課題への直接的な応用可能性が見えにくい。
...
**邦訳:** KLIP: 拡散事前分布を用いたKL発散による逆問題における局所化分布シフト検出
**Authors:** Alireza Kheirandish, Jihoon Hong, Sara Fridovich-Keil
**Category:** cs.CV
**Published:** 2026-05-29
**arXiv:** https://arxiv.org/abs/2605.31596

**評価理由:** 拡散モデルを用いた逆問題のOOD検出という技術的に興味深い研究だが、医療画像（肝臓CT）への応用例は限定的であり、都市・交通・社会データ・SNS・政策などの応用領域との接続が明確でない。応用研究者にとっては、方法論の転用可能性は中程度だが、主流の関心領域からは外れている。
...
**邦訳:** 言語モデルは構文的意味論を学習する：ペアフォーカス構文に対するLMの理解を調査
**Authors:** Wesley Scivetti, Ethan Wilcox, Nathan Schneider, Kanishka Misra, Leonie Weissweiler
**Category:** cs.CL
**Published:** 2026-05-29
**arXiv:** https://arxiv.org/abs/2605.31586

**評価理由:** LLMの構文・意味理解に関する言語学的な基礎研究であり、技術的には興味深いが、社会・都市・交通・政策などの応用接続が全く見当たらない。応用研究者にとっては優先度が低い。
...
**邦訳:** 位置的注意ヘッドと記号的注意ヘッド：学習ダイナミクス、RoPE幾何学、長さ外挿性
...
**評価理由:** Transformerの注意メカニズムの理論的解析であり、技術的には興味深いが、応用接続（社会・都市・交通・SNS・政策など）が明示的でなく、純粋な機械学習理論に偏っている。長さ外挿性の知見は有用だが、応用研究者にとっての実践的な価値が限定的。
...
**邦訳:** 曖昧な入力下でビジョン言語モデルが女性表現を抑制する
**Authors:** Arnau Marin-Llobet, Simon Henniger, Mahzarin R. Banaji
**Category:** cs.CV
**Published:** 2026-05-29
**arXiv:** https://arxiv.org/abs/2605.31556

**評価理由:** VLMのジェンダーバイアス分析は興味深いが、応用接続（都市・交通・社会データ・政策など）が明示的でなく、純粋なAI倫理・バイアス分析に留まっている。社会的影響の議論はあるが、実装や政策への具体的な応用可能性が見えにくい。
...
**邦訳:** Lumos-Nexus: ビデオ統合モデルのための同次潜在空間を用いた効率的周波数ブリッジング
...
**評価理由:** 動画生成の品質改善が主目的であり、応用接続（都市・交通・社会データ・政策など）が明示されていない。画像/動画生成の視覚品質向上はプロファイルのスキップ対象に該当する。
...
**邦訳:** 分散最適化における誤差フィードバックアルゴリズムの厳密な理論
**Authors:** Daniel Berg Thomsen, Adrien Taylor, Aymeric Dieuleveut
**Category:** cs.LG
**Published:** 2026-05-29
**arXiv:** https://arxiv.org/abs/2605.31594

**評価理由:** 分散最適化における勾配圧縮と誤差フィードバックの理論解析であり、通信効率改善は実用的だが、社会データ・都市・交通・政策などの応用接続が全く見えない純粋な最適化理論論文である。応用研究者にとって優先度は低い。
...
**評価理由:** 論文は議論フレームワークの理論的拡張（CDAFs）を扱っており、戦略的操作の概念は興味深いが、社会・都市・交通・政策などの具体的な応用接続が全く見当たらず、純粋な形式論理・計算複雑性の理論研究に留まっている。応用研究者にとって優先度は低い。
...
**邦訳:** SPECTRA: 関連性オラクルと制御可能な妨害診断を備えた合成IR テストコレクション
**Authors:** Eric Liang
**Category:** cs.IR
**Published:** 2026-05-29
**arXiv:** https://arxiv.org/abs/2605.31575

**評価理由:** 情報検索のテスト用コーパス生成フレームワークであり、技術的には堅実だが、応用領域（都市・交通・社会データ・政策など）への接続が全く見えず、純粋なIR技術論文に該当するため、応用研究者にとっての優先度は低い。
...
**評価理由:** 生物学的データ（遺伝子発現）の表現学習に関する技術論文だが、応用接続が都市・交通・社会データ・政策・人間行動などの研究者の主要関心領域と直結していない。医療・創薬応用は言及されているが、研究者の専門分野（都市計画・交通計画・社会データ分析）との接続が明確でなく、優先度は低い。
...
**評価理由:** 純粋な演算子学習・関数空間の理論的改善が主で、応用接続（都市・交通・社会データ・政策）が明示されていない。PDE求解や3D分割は技術的に興味深いが、応用研究者の優先度では低い。
...
**邦訳:** カオス代理モデリングにおける動的確率的一貫性ギャップ
**Authors:** Andre Herz, Matthijs Pals, Daniel Durstewitz, Georgia Koppe
**Category:** cs.LG
**Published:** 2026-05-29
**arXiv:** https://arxiv.org/abs/2605.31547

**評価理由:** カオス系の力学系再構成における不確実性推定の理論的問題を扱う純粋な技術論文。数学的に高度だが、社会・都市・交通・SNS・政策などの応用接続が全く見えず、応用研究者にとっての優先度は低い。
...
**邦訳:** 術前CT画像を用いた膵瘻の自動予測：深層学習パイプラインの開発
...
**評価理由:** 医療AI応用研究だが、都市・交通・社会データ・政策・SNS・人間行動などの応用領域との接続がなく、応用研究者の優先度基準に合致しない。医療画像分類の性能改善が主目的であり、クロスドメイン性も低い。
...
**評価理由:** 強化学習と形式検証の理論的接続は興味深いが、純粋な理論解析が中心で、都市・交通・社会データなどの応用接続が全く見えない。応用研究者にとっての実用性が不明確。
...
**邦訳:** UniAudio-Token: 一般的な音声認識能力で意味的音声トークナイザーを強化する
...
**評価理由:** 音声トークン化の技術改善論文だが、応用接続（都市・交通・社会データ・政策など）が明示的でなく、Audio-LLMの性能向上が主目的。応用研究者にとっての実装価値が不明確。
...
**評価理由:** セキュリティ技術（認証情報漏洩検出）の改善論文だが、都市・交通・社会データ・政策・人間行動などの応用領域との接続がなく、純粋なセキュリティエンジニアリング問題に留まっている。応用研究者の優先度基準に該当しない。
...
**邦訳:** Sparse Autoencodersにおける活性化外れ値と特徴死滅の関係
**Authors:** Elana Simon, Etowah Adams, James Zou
**Category:** cs.LG
**Published:** 2026-05-29
**arXiv:** https://arxiv.org/abs/2605.31518

**評価理由:** Sparse Autoencoders（SAE）の技術的な改善論文だが、応用接続が明確でなく、純粋なニューラルネットワーク解釈可能性の理論研究に該当。都市・交通・社会データ分析への応用可能性が見えにくいため、応用研究者の優先度としては低い。
...
**評価理由:** 動画生成の品質改善が主目的であり、社会・都市・交通・政策などの応用接続が見えない。画像/動画生成の狭いベンチマーク改善系に該当するため、スキップ対象。
...
**評価理由:** 3D視点合成の技術的改善論文であり、応用接続（都市・交通・社会データ・政策）が明示的に見当たらない。画像生成・3D生成品質改善系に該当し、スキップ対象。
...
**評価理由:** 3Dシーン再構成の技術論文だが、都市・交通・社会データ分析への応用接続が全く見えず、純粋なコンピュータビジョンの性能改善に留まる。応用研究者にとって優先度は極めて低い。
...
**評価理由:** 材料科学における熱力学的に許容される散逸ポテンシャルの発見に関する純粋な理論・計算研究であり、都市・交通・社会データ・SNS・政策などの応用領域への接続が全く見当たらない。応用研究者にとって関連性が低い。
...
**邦訳:** Chem-PerturBridge：小分子摂動トランスクリプトミクス効果の調和化コンペンディウム
...
**評価理由:** 化学物質の摂動トランスクリプトミクスデータの統合リソース構築と機械学習モデル評価に関する論文だが、応用研究者の関心領域（都市・交通・社会データ・政策・人間行動）との接続が全く見えない。生物医学分野の専門的な技術論文であり、社会応用や都市・交通・SNS分析への転用可能性が極めて低い。
